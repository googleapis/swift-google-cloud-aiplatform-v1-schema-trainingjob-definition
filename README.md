...
Add `swift-google-cloud-aiplatform-v1-schema-trainingjob-definition` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-aiplatform-v1-schema-trainingjob-definition.git --from 0.0.0-preview
```

Then add `GoogleCloudAIPlatformV1SchemaTrainingJobDefinition` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAIPlatformV1SchemaTrainingJobDefinition <target-name> --package swift-google-cloud-aiplatform-v1-schema-trainingjob-definition
```
